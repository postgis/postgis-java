 ...
 * org.postgresql.Driver. When using a J2EE DataSource implementation, set the driver class property in the datasource
 * config, the following works for jboss:
 *
 * <code>
 * &lt;driver-class&gt;org.postgis.DriverWrapper&lt;/driver-class&gt;
 * </code> If you don't like or want to use the DriverWrapper, you have two alternatives, see the README file.
 *
 * Also note that the addDataType() methods known from earlier pgjdbc versions are deprecated in pgjdbc 8.0, see the
 * commented code variants in the addGisTypes() method.
 *
 * This wrapper always uses EWKT as canonical text representation, and thus works against PostGIS 1.x servers as well as
 * 0.x (tested with 0.8, 0.9 and 1.0).
 *
...
public class DriverWrapper extends Driver {
  ...
  public DriverWrapper() throws SQLException {
    super();
    this.typesAdder = getTypesAdder(this);
    // The debug method is @since 7.2
    if (super.getMajorVersion() > 8 || super.getMinorVersion() > 1) {
      logger.fine(this.getClass().getName() + " loaded TypesAdder: " + this.typesAdder.getClass().getName());
    }
  }
  ...
  public java.sql.Connection connect(String url, Properties info) throws SQLException {
    ...
    this.typesAdder.addGT(result, useLW(result));
    return result;
  }
  ...
  /*
   * Here follows the addGISTypes() stuff. This is a little tricky because the pgjdbc people had several, partially
   * incompatible API changes during 7.2 and 8.0. We still want to support all those releases, however.
   *
   */
  /**
   * adds the JTS/PostGIS Data types to a PG 7.3+ Connection. If you use PostgreSQL jdbc drivers V8.0 or newer, those
   * methods are deprecated due to some class loader problems (but still work for now), and you may want to use the
